...
export const register = async (req, res) => {
  try {
    ...
    console.log(`otp ${otp}`);
    console.log(
      `cloudingary name ${process.env.CLOUD_NAME} apikey ${process.env.CLOUD_API_KEY}  apisecret ${process.env.CLOUD_API_SECRET}`
    );

    const mycloud = await cloudinary.v2.uploader.upload(
      avatar,
      function (error, result) {
        console.log(` this is error ${error} and this is result ${result}`);
      }
    );
